const PRODUCTS=[
{id:'bb001',name:'Stay Rare Oversized Tee',price:2499,category:'streetwear'},
{id:'bb002',name:'Tokyo Nights Graphic Tee',price:2699,category:'anime'},
{id:'bb003',name:'Matchday Oversized Tee',price:2499,category:'football'},
{id:'bb004',name:'Archive Washed Tee',price:2899,category:'vintage'},
{id:'bb005',name:'Broken Beyond Tee',price:2599,category:'streetwear'},
{id:'bb006',name:'Neo Tokyo Tee',price:2799,category:'anime'},
{id:'bb007',name:'90s Football Tee',price:2499,category:'football'},
{id:'bb008',name:'Rare Signal Tee',price:2999,category:'vintage'}
];
...
function addToCart(id){const p=PRODUCTS.find(x=>x.id===id);if(!p)return;const c=getCart();const item=c.find(x=>x.id===id);item?item.qty++:c.push({...p,qty:1});saveCart(c);showToast('Added to cart');}
...
document.querySelector('#searchBtn')?.addEventListener('click',()=>{searchModal.hidden=false;document.querySelector('#searchInput')?.focus()});
document.querySelector('#closeSearch')?.addEventListener('click',()=>searchModal.hidden=true);
