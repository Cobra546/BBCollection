...
async function syncCart(cart=getCart()){try{const sb=await bbReady;const u=await currentUser();if(!u)return;await sb.from('cart_items').delete().eq('user_id',u.id);if(cart.length)await sb.from('cart_items').insert(cart.map(x=>({user_id:u.id,product_id:Number(x.id),quantity:x.qty})));}catch(e){console.warn('Cart sync:',e)}}
async function loadAccountCart(){try{const sb=await bbReady;const u=await currentUser();if(!u)return getCart();const {data,error}=await sb.from('cart_items').select('product_id,quantity').eq('user_id',u.id);if(error)throw error;const cloud=(data||[]).map(x=>{const p=PRODUCTS.find(y=>y.id===String(x.product_id));return p?{...p,qty:x.quantity}:null}).filter(Boolean);localStorage.setItem('bb_cart',JSON.stringify(cloud));updateCartCount();return cloud}catch(e){console.warn('Cart load:',e);return getCart()}}
async function addToCart(id){const p=PRODUCTS.find(x=>x.id===String(id));if(!p)return;const c=getCart();const item=c.find(x=>x.id===p.id);item?item.qty++:c.push({...p,qty:1});saveCart(c);showToast('Added to cart');}
