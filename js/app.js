...
async function loadAccountCart(){try{await cartSyncPromise;const sb=await bbReady;const u=await currentUser();if(!u)return getCart();const {data,error}=await sb.from('cart_items').select('product_id,quantity,print_name').eq('user_id',u.id);if(error)throw error;const cloud=(data||[]).map(x=>{const p=PRODUCTS.find(y=>y.id===String(x.product_id));return p?{...p,qty:x.quantity,print_name:x.print_name||''}:null}).filter(Boolean);localStorage.setItem('bb_cart',JSON.stringify(cloud));updateCartCount();return cloud}catch(e){console.warn('Cart load:',e);return getCart()}}
async function addToCart(id,printName=''){const p=PRODUCTS.find(x=>x.id===String(id));if(!p)return;const print=String(printName||'').trim().slice(0,100);const c=getCart();const item=c.find(x=>x.id===p.id&&(x.print_name||'')===print);item?item.qty++:c.push({...p,qty:1,print_name:print});await saveCart(c);showToast(print?`Added with print: ${print}`:'Added to cart');return c;}
