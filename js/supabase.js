...
window.bbOrderApi = {
  async create(order, items) {
    ...
    return window.bbSupabase.rpc('place_order', {
  ...
  async getAdminOrders() {
    if (!window.bbSupabase) return { data: null, error: new Error('Supabase is not ready yet.') };
    return window.bbSupabase.from('orders').select('*, order_items(*)').order('created_at', { ascending: false });
  },

  async updateStatus(id, status) {
    if (!window.bbSupabase) return { data: null, error: new Error('Supabase is not ready yet.') };
    return window.bbSupabase.from('orders').update({ status }).eq('id', id).select().single();
  }
};
